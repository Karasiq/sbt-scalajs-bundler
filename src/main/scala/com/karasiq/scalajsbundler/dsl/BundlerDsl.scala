...
trait BundlerDsl {
  object PageContentBuilders {
    ...
    case class InlineBuilder[+C <: PageTypedContent](ext: String, mime: String, f: (Asset, String, String) => C)
        extends TypedContentBuilder[C, InlineBuilder[C]] {
      ...
      override def fromAsset(asset: Asset): C =
        f(asset, ext, mime)
    }

    case class FileBuilder(name: String, ext: String, mime: String = "application/octet-stream")
        extends TypedContentBuilder[PageFile, FileBuilder] with StaticContentBuilder[PageFile, FileBuilder] {
      override def withName(name: String): FileBuilder =
        copy(name = name)

      override def withExt(ext: String): FileBuilder =
        copy(ext = ext)

      override def withMime(mime: String): FileBuilder =
        copy(mime = mime)

      override def fromAsset(asset: Asset): PageFile =
        PageFile(this.name, asset, this.ext, this.mime)
    }
  }
